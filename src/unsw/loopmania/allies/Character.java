...
public class Character extends MovingEntity {
    ...
    private transient SimpleIntegerProperty gold;
    private transient SimpleIntegerProperty exp;
    private transient SimpleStringProperty healthTxt;
    ...
    private int stun;
    String ImageUrl = "src/images/human_new.png";
    String ToolTipString = "This is You \nAlone in the dark durgeon \nPathetic :(";

    // TODO = potentially implement relationships between this class and other
    // classes
    public Character(PathPosition position) {
        super(position);
        health = new SimpleIntegerProperty(1000); // should be 1000
        maxhealth = new SimpleIntegerProperty(1000); // should be 1000
    ...
    /**
     * Update character's max health while gain exp
     * 
     * @param newHealth
     */
    public void setMaxHealth(Integer newMaxHealth) {
    ...
    /**
     * Assert if character is stunned
     * 
     * @return
     */
    ...
    /**
     * character gradually recover from stun
     */
    ...
    /**
     * Character equips TreeStump and reduces boss dmg
     */
    public void equipTreeStump() {
        System.out.println("Player equips TreeStump");
        shield = 2;
    }
     ...
     * @return Boolean shield
     */
    public Boolean ifTreeStump() {
        return shield == 2;
    }

    /**
     * EquipSword on Character, modifies AttackStrategy to SwordStrategy
     */
    ...
    /**
     * EquipRapier on Character, modifies AttackStrategy to RapierStrategy
     */
    public void equipRapier() {
        System.out.println("Player equips Rapier");
        attackMethod = new RapierStrategy();
    }

    /**
     * EquipStake on Character, modifies AttackStrategy to StakeStrategy
     */
    ...
    /**
     * EquipStaff on Character, modifies AttackStrategy to StaffStrategy
     */
    ...
    /**
     * UnEquip Weapon on Character, modifies AttackStrategy to FistStrategy
     */
    ...
    /**
     * Make soldier strictly follows character; In case, the first one dies, the
     * second and the third would move one additional step to stay right after
     * character icon. Also, removes dead soldier from list.
     */
    ...
    /**
     * Let Trance enemy to attack other enemy; Remove enemy from list if enemy exits
     * Trance state
     * 
     * @param e
     * @param rd
    ...
    /**
     * Run battle against enemy; However vampire attacks first; Character usually
     * attack first against other enemy; Vampire with its larger support radius can
     * ambush player, by attacking first twice;
     * 
     * @param e
     * @param rd
     * @param w
    ...
    public void runBattle(BasicEnemy e, Random rd, LoopManiaWorld w, List<Building> buildingEntities) {
        ...
        if (e.getType().get().equals("Vampire")) {
            ...
            br = b.getBattleRadius();
            sr = b.getSupportRadius();
            if (!b.ifDead() && !this.ifDead()
                    && Math.pow((this.getX() - b.getX()), 2) + Math.pow((this.getY() - e.getY()), 2) <= (br * br)) {
                // other enemy attacks after this
                while (!b.ifDead() && !this.ifDead()) {
                    this.triggerBattle();
                    this.attack(b, rd.nextInt(100));
                    b.attack(this, rd.nextInt(100), w);
                    this.soldierAttack(b, rd.nextInt(100), w);
                    this.towerAttack(b, rd.nextInt(100), buildingEntities);
                    this.tranceAttack(b, rd.nextInt(100), w);
                }
            }
            b.triggerDefeatFlag(w);
        }
    }
