...
public class Zombie extends BasicEnemy {
    ...
    private int trance;
    String ImageUrl = "src/images/zombie.png";
    String ToolTipString = "Zombieeeeeeeeeeeee!\nZombies have low health, moderate damage, and are slower compared to other enemies. \nA critical bite from a zombie against an allied soldier (which has a random chance of occurring) \nwill transform the allied soldier into a zombie, which will then proceed to fight \nagainst the Character until it is killed. Zombies have a higher battle radius than slugs";

    public Zombie(PathPosition position) {
        super(position);
        health = new SimpleIntegerProperty(20);
        maxhealth = new SimpleIntegerProperty(20);
    ...
    /**
     * Assert if Zombie is dead or tranced to prevent being attacked by Character /
     * Soldier / enemy
     * 
     * @return Boolean
     */
    ...
    public Boolean setZombiePosition(int zombieCount, int zombiePitCount, List<Building> buildingEntities) {
        Pair<Integer, Integer> zombiePos = possiblyGetZombieSpawnPosition(zombieCount, zombiePitCount,
                buildingEntities);
        if (zombiePos != null) {
     ...
     */
    public Pair<Integer, Integer> possiblyGetZombieSpawnPosition(int zombieCount, int zombiePitCount,
