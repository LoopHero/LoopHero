 ...
 */
public class LoopManiaWorld implements Serializable {
    ...
    // added loop counter
    private transient FloatProperty loop;
    private int moves;
    ...
    // doggie and elon defeat flag
    private boolean doggieDefeat;
    private boolean elonDefeat;
    ...
    public void showGameMode() {
        switch (mode) {
            ...
            case 2:
                ...
                break;
            case 3:
                System.out.println("Gamemode: ConfusionMode");
                break;
            default:
    ...
    public void ElonDefeated() {
        elonDefeat = true;
    }

    public boolean doDoggieCoinPriceDrop() {
        return elonDefeat;
    }

    public void doggieDefeated() {
        doggieDefeat = true;
    }

    public boolean doDoggieCoinSpawn() {
        return doggieDefeat;
    }
    ...
    /**
     * setGameMode to Confusion
     * 
     * @return
     */
    public void setGameModeConfusion() {
        mode = 3;
    }
    ...
    /**
     * assert if gamemode is Confusion
     * 
     * @return
     */
     ...
     */
    public void spawnDoggie(List<BossEnemy> spawningEnemies) {
        Doggie doggie = new Doggie(new PathPosition(0, orderedPath));
        doggie.setOrderedPath(orderedPath);
        if ((doggie.canSpawn(this) && !doggieDefeat) || (doggie.canSpawn(this) && rd.nextInt(100) < 5)) {
            Boolean doggieSpawned = doggie.setDoggiePosition(character.getX(), character.getY(), countDoggie());
            if (doggieSpawned) {
                enemies.add(doggie);
     ...
     * @param spawningEnemies
     */
    public void spawnElonMask(List<BossEnemy> spawningEnemies) {
        ElonMask em = new ElonMask(new PathPosition(0, orderedPath));
        em.setOrderedPath(orderedPath);
        if ((em.canSpawn(this) && (!elonDefeat || rd.nextInt(100) < 3))) {
            Boolean emSpawned = em.setElonPosition(character.getX(), character.getY(), countElonMask());
            if (emSpawned) {
                enemies.add(em);
                spawningEnemies.add(em);
            }
        }
        return;
    }
    ...
    public void spawnVampire(List<BasicEnemy> spawningEnemies) {
        for (int i = 0; i < buildingEntities.size(); i++) {
            if (buildingEntities.get(i) instanceof VampireCastleBuilding) {
                ...
                Boolean vampireSpawned = vampire.setVampirePosition(countVampires(), countVampireCastles(),
                        buildingEntities);
                if (vampireSpawned) {
                    enemies.add(vampire);
    ...
    public List<BossEnemy> possiblySpawnBosses() {
        List<BossEnemy> spawningEnemies = new ArrayList<BossEnemy>();
        spawnDoggie(spawningEnemies);
        spawnElonMask(spawningEnemies);
        return spawningEnemies;
    }
    ...
    public void runBattles() {
        ...
        for (int i = 0; i < enemies.size(); i++) {
            ...
            // Some boss has aura effect on basic enemies
    ...
    private Item convertNametoItem(SimpleIntegerProperty x, SimpleIntegerProperty y, String itemName) {
        switch (itemName) {
            ...
            case "Rapier":
                return new Rapier(x, y, itemName);
            case "TreeStump":
                return new TreeStump(x, y, itemName);
            case "DoggieCoin":
                return new DoggieCoin(x, y, itemName);
            case "Potion":
    ...
    public Item addUnequipedItem(String itemName) {
        ...
        // let rare item to have a second property in confusion mode
        if (item instanceof RareItem && isConfusionMode()) {
            RareItem rt = (RareItem) item;
            rt.setHidden(rd.nextInt(2));
        }
    ...
    public Entity getEquippedInventoryItemEntityByCoordinates(int x, int y) {
        ...
        for (Entity item : equippedInventoryItems) {
            ...
            if (x == 0 && y == 1 && (item instanceof Sword || item instanceof Staff || item instanceof Stake))
                return item;
            if (isConfusionMode() && item instanceof RareItem) {
                RareItem rt = (RareItem) item;
                if (x == 2 && y == 1 && rt.getHidden() instanceof TreeStump) {
                    return rt;
                }
                if (x == 0 && y == 1 && rt.getHidden() instanceof Rapier) {
                    return rt;
                }
            }
        }
     ...
     * @return
     */
    public Building convertTypetoBuilding(SimpleIntegerProperty x, SimpleIntegerProperty y, String type) {
     ...
     * @return
     */
    public Card convertCardIndextoCard(int cardNodeX, int cardNodeY) {
        for (Card c : cardEntities) {
    ...
    public boolean hasOneRing() {
        for (int i = 0; i < unequippedInventoryItems.size(); i++) {
            ...
            // confusion mode addition
            if (isConfusionMode() && se instanceof RareItem) {
                RareItem rt = (RareItem) se;
                if (rt.getHidden() instanceof TheOneRing) {
                    return true;
                }
            }
        }
     ...
     * @return
     */
    public boolean checkEquipSlot(Entity item, int x, int y, SimpleBooleanProperty hidden) {
        // Armour (1, 1)
        // Helmet (1, 0)
        // Shield (2, 1)
        // Weapon (0, 1)
        ...
        if (x == 2 && y == 1 && (item instanceof Shield || item instanceof TreeStump)) {
            return true;
        }
        if (x == 0 && y == 1 && (item instanceof Sword || item instanceof Staff || item instanceof Stake
     ...
     * @return
     */
    public boolean canEquipItem(Entity item, int x, int y, GridPane targetGridPane,
            HashMap<Entity, ImageView> equipItemsRecord, SimpleBooleanProperty hidden) {
        if (!checkEquipSlot(item, x, y, hidden))
            return false;
        boolean armour = false;
        boolean helmet = false;
        boolean shield = false;
        boolean weapon = false;
        // identify terget with x and y
        // Armour (1, 1)
        // Helmet (1, 0)
        // Shield (2, 1)
        // Weapon (0, 1)
        for (Entity equippedItem : equippedInventoryItems) {
            if (equippedItem instanceof Armour)
                armour = true;
            if (equippedItem instanceof Helmet)
                helmet = true;
            if (equippedItem instanceof Shield || equippedItem instanceof TreeStump) { // added TreeStump
                shield = true;
            }
            if (equippedItem instanceof Sword || equippedItem instanceof Staff || equippedItem instanceof Stake
                    || equippedItem instanceof Rapier) { // added Rapier
                weapon = true;
            }
            // added confusion mode
            if (isConfusionMode() && equippedItem instanceof RareItem) {
                RareItem ert = (RareItem) equippedItem;
                if (ert.getHidden() instanceof TreeStump) {
                    shield = true;
                }
                if (ert.getHidden() instanceof Rapier) {
                    weapon = true;
                }
            }
        }
        if (item instanceof Armour && armour) {
            for (Entity equippedItem : equipItemsRecord.keySet()) {
                if (equippedItem instanceof Armour) {
                    targetGridPane.getChildren().remove(equipItemsRecord.get(equippedItem));
                    // System.out.println("Removing previous Armour");
                }
            }
        }
        if (item instanceof Helmet && helmet) {
            for (Entity equippedItem : equipItemsRecord.keySet()) {
                if (equippedItem instanceof Helmet) {
                    targetGridPane.getChildren().remove(equipItemsRecord.get(equippedItem));
                    // System.out.println("Removing previous Helmet");
                }
            }
        }
        if ((item instanceof Shield || item instanceof TreeStump) && shield) {
            for (Entity equippedItem : equipItemsRecord.keySet()) {
                if (equippedItem instanceof Shield || equippedItem instanceof TreeStump) {
                    targetGridPane.getChildren().remove(equipItemsRecord.get(equippedItem));
                    // System.out.println("Removing previous Shield/TreeTrump");
                }
                // remove item if it has a secondary property as TreeStump
                if (isConfusionMode() && !(equippedItem instanceof TreeStump) && equippedItem instanceof RareItem) {
                    RareItem ert = (RareItem) equippedItem;
                    if (ert.getHidden() instanceof TreeStump) {
                        targetGridPane.getChildren().remove(equipItemsRecord.get(equippedItem));
                        // System.out.println("Removing previous Shield/TreeTrump");
        ...
        if ((item instanceof Sword || item instanceof Staff || item instanceof Stake || item instanceof Rapier)
                && weapon) {
            for (Entity equippedItem : equipItemsRecord.keySet()) {
                if (equippedItem instanceof Sword || equippedItem instanceof Staff || equippedItem instanceof Stake
                        || equippedItem instanceof Rapier) {
                    targetGridPane.getChildren().remove(equipItemsRecord.get(equippedItem));
                    // System.out.println("Removing previous Weapon");
                }
                if (isConfusionMode() && !(equippedItem instanceof Rapier) && equippedItem instanceof RareItem) {
                    RareItem ert = (RareItem) equippedItem;
                    if (ert.getHidden() instanceof Rapier) {
                        targetGridPane.getChildren().remove(equipItemsRecord.get(equippedItem));
                        // System.out.println("Removing previous Shield/TreeTrump");
        ...
        // confusion mode, character can only equip one rare item at any time
        if (isConfusionMode() && item instanceof RareItem) {
            // remove equiped rare item
            RareItem rt = (RareItem) item;
            for (Entity equippedItem : equipItemsRecord.keySet()) {
                if (equippedItem instanceof RareItem
                        || (rt.getHidden() instanceof Rapier
                                && (equippedItem instanceof Sword || equippedItem instanceof Staff
                                        || equippedItem instanceof Stake || equippedItem instanceof Rapier))
                        || (rt.getHidden() instanceof TreeStump
                                && (equippedItem instanceof Shield || equippedItem instanceof TreeStump))) {
                    targetGridPane.getChildren().remove(equipItemsRecord.get(equippedItem));
                    // System.out.println("Removing equiped rare items");
                }
            }
        }
        return true;
    ...
    /**
     * Show if boss has been defeated yet
     */
    public void showBossState() {
        if (doggieDefeat) {
            System.out.println("Doggie has been defeated once or more times;");
        } else {
            System.out.println("Doggie has not been defeated yet;");
        }
        if (elonDefeat) {
            System.out.println("ElonMask has been defeated once or more times;");
        } else {
            System.out.println("ElonMask has not been defeated yet;");
        }
    }
}
